...
/*   SlowStreak = True;
   ...
   if ( (MaPrevious2 >= FastMa2) && (FastMa1 >= MaPrevious1) && uptrend)
   {
      buySignal = true;
   }
   //fast MA crosses below slow MA.
   else if (MaPrevious2 <= FastMa2 && FastMa1 <= MaPrevious1 && downtrend)
   {
      sellSignal = true;
   }
   
      //Price moves below MA or the price moves below the low of the last (negative) bar
   else if ( CClose  >= iBands(NULL,0,BBandsPeriod,BBandsSD,0,PRICE_CLOSE,MODE_UPPER,0) || (MaPrevious2 < FastMa2 && FastMa1 < MaPrevious1 ))
   {
      CloseBuySignal = true;
   }
        //Price moves above ma, or price moves above the high of the last positive bar
   else if ( CClose  <= iBands(NULL,0,BBandsPeriod,BBandsSD,0,PRICE_CLOSE,MODE_LOWER,0) || (MaPrevious2 > FastMa2 && FastMa1 > MaPrevious1))
   {
