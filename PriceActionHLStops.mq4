...
extern double takeProfitFactor = 2;
...
void placeSell()
{
   ...
   if (takeProfitFactor > 0) TP = NormalizeDouble(Bid - Point*stopLoss*takeProfitFactor,Digits());
...
// if the stop level is below the minimum stop distance allowed, reconfigure
if (stopLoss <StopLevel){
   stopLoss = StopLevel*2;
   Print("Order s/l set at min : ",StopLevel, " not ", stopLoss) ;
   }
